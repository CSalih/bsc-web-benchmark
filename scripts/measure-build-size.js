    ...
    .map((file) => {
      ...
      if (file.isFile() && (file.name.endsWith(".js") || file.name.endsWith(".wasm"))) {
        ...
        if (file.name.endsWith(".js") || file.name.endsWith(".wasm")) {
          ...
          const brotliSize = parseInt(execSync(`brotli -6 --stdout ${filePath} | wc --bytes`).toString().trim());
          fileList.push({
            filePath,
            size: size,
            gzipSize: gzipSize,
  ...
  return fs.readdirSync(directory)
    ...
    .map((filePath) => {
      const size = fs.existsSync(filePath)
        ? directorySizeRecursive(filePath, 0)
        : -1
      return {
        filePath,
        size: size,
...
console.table(fileList.map((file) => ({
